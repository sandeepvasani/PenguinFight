...
-(id) init {
	if ( (self=[super init]) ) {
        ...
        scrollOffsets=[NSMutableArray arrayWithCapacity:5];
	}
	return self;
}
...
-(void) updateWithVelocity:(CGPoint)vel AndDelta:(CCTime)dt {
	...
	for (int i=scrollOffsets.count - 1; i >= 0; i--) {
		CCParallaxScrollOffset *scrollOffset = [scrollOffsets objectAtIndex:i];
...
-(void) removeAllChildrenWithCleanup:(BOOL)cleanup {
    ...
    [scrollOffsets removeAllObjects];
	if (batch) {
...
-(void) addInfiniteScrollWithObjects:(NSMutableArray*)objects Z:(NSInteger)z Ratio:(CGPoint)ratio Pos:(CGPoint)pos Dir:(CGPoint)dir {
...
-(void) addInfiniteScrollWithZ:(NSInteger)z Ratio:(CGPoint)ratio Pos:(CGPoint)pos Dir:(CGPoint)dir Objects:(CCSprite*)firstObject, ... {
	...
	NSMutableArray* argArray = [NSMutableArray arrayWithCapacity:2];
